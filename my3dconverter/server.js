...
// Remove background using BiRefNet v2 - Submit job (password protected)
app.post('/api/remove-background', requirePassword, upload.single('image'), async (req, res) => {
    try {
        ...
        console.log('🚀 Submitting to BiRefNet v2 queue...');

        // Submit to BiRefNet v2 queue
        ...
        if (!submitRes.ok) {
            ...
            console.error('❌ Submit failed:', submitRes.status, errText);
            return res.status(submitRes.status).json({ 
                error: `Background removal submit failed: ${submitRes.statusText}`,
                details: errText
            });
        }
        ...
        console.log('✅ Background removal job submitted! ID:', requestId);

        // Return job ID to frontend
        res.json({ 
            success: true, 
            requestId,
            message: 'Background removal job submitted'
        });
...
// Check background removal status (password protected)
app.get('/api/bg-status/:requestId', requirePassword, async (req, res) => {
    try {
        ...
        const { requestId } = req.params;
        console.log('🔍 Checking BiRefNet status for:', requestId);
        ...
        const statusData = await statusRes.json();
        console.log('📊 BiRefNet Status:', statusData.status);

        res.json(statusData);

    } catch (error) {
        console.error('❌ BiRefNet status check error:', error);
        res.status(500).json({ error: error.message });
    }
});

// Get background removal result (password protected)
app.get('/api/bg-result/:requestId', requirePassword, async (req, res) => {
    try {
        ...
        const { requestId } = req.params;
        console.log('📥 Fetching BiRefNet result for:', requestId);
        ...
        console.log('✅ BiRefNet result retrieved!');
        console.log('Result structure:', JSON.stringify(resultData, null, 2));

        res.json(resultData);

    } catch (error) {
        console.error('❌ BiRefNet result fetch error:', error);
        res.status(500).json({ error: error.message });
    }
});
